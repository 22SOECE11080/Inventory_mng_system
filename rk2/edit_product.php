...
if ($result->num_rows > 0) {
    ...
    if (isset($_GET['edit_product'])) {
        ...
        $queryProduct = "SELECT * FROM products WHERE p_id = '$product_id'";
        $resultProduct = $conn->query($queryProduct);

        if ($resultProduct->num_rows > 0) {
            $productInfo = $resultProduct->fetch_assoc();
            $p_name = $productInfo['p_name'];
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                $updateQuery = "UPDATE products SET p_name = '$p_name', price = '$price', quantity = '$quantity', status = '$status' WHERE p_id = '$product_id'";
