...
if (isset($_SESSION['forgot_em'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM agency WHERE user_username = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        if ($stmt_insert->execute()) {
            ...
            try {
                ...
                $mail->setFrom('rnv1924@gmail.com', 'Radhesh');
                $mail->addAddress($em, $row['a_name']);
                ...
                if ($mail->send()) {
                    ...
                    header("Location: http://localhost/Inventory_mng_system/user_panel/Forgot_password_otp.php");
                    exit;
                } else {
                    setcookie("error", "Error in sending OTP. Please try again later.", time() + 120, "/");
                    header("Location: http://localhost/Inventory_mng_system/user_panel/forget_password.php");
                    exit;
                }
            } catch (Exception $e) {
                setcookie("error", "Error in sending OTP. Please try again later.", time() + 120, "/");
